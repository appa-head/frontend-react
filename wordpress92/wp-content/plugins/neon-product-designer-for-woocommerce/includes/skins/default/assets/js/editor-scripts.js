  ...
  $(document).ready(function () {
    ...
    function initialise_editor() {
     if(window.npd.font_behaviour == "text"){
      ...
      var npd_txt_height = $("input[name='npd-txt-size']:checked").attr("data-height");
      var npd_txt_width = $("input[name='npd-txt-size']:checked").attr("data-width");
      var editor_size = parseInt(change_editor_size(npd_txt_width, npd_txt_height));
    ...
    window.npd_text_dimensions = function () {
      ...
      var height = $('#npd-text-render-wrap').height();
      return [width,height];
    };
    npd_text_dimensions();
    ...
    function change_editor_size(neon_w, neon_h) {
      neon_w = convert_to_px(neon_w, npd_unit) * 0.1;
      neon_h = convert_to_px(neon_h, npd_unit);
      var original_w = $(".npd-text-render-container").width(),
          max_w = $(".npd-editor-preview").width();
      var npd_text_size = $("#npd-text-render-wrap").text().length;

      if(neon_w <= max_w) original_w = neon_w;
      var npd_font_size = (original_w / npd_text_size) * 2;
      var text_width = original_w - npd_font_size;
      return npd_font_size;
    }
