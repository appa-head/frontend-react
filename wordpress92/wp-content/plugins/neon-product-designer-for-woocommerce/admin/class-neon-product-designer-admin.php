...
class Neon_Product_Designer_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->npd, plugin_dir_url( __FILE__ ) . 'css/neon-product-designer-admin.css', array(), $this->version, 'all' );
		wp_enqueue_style( 'select2-css', plugin_dir_url( __FILE__ ) . 'css/select2.min.css', array(), $this->version, 'all' );
		wp_enqueue_style( 'modal-css', plugin_dir_url( __FILE__ ) . 'js/modal/modal.min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'SpryTabbedPanels-js',
			plugin_dir_url( __FILE__ ) . 'js/SpryTabbedPanels.min.js',
			array( 'jquery' ),
			$this->version,
			'all'
		);
		...
		wp_enqueue_script(
			'modal-js',
			plugin_dir_url( __FILE__ ) . 'js/modal/modal.min.js',
			array( 'jquery' ),
			$this->version,
			'all'
		);
	...
	private function get_general_settings() {
		...
		return vc_admin_fields( $options );
	}
	...
	function notify_customization_page_missing() {
		...
		$hide_notices = vc_get_proper_value( $options, 'hide-requirements-notices', false );
	...
	function notify_minmimum_required_parameters() {
		global $npd_settings;
		$general_options = vc_get_proper_value( $npd_settings, 'npd-general-options' );
		$hide_notices    = vc_get_proper_value( $general_options, 'hide-requirements-notices', false );
		$allowed_html = array(
		    'a'      => array(
		        'href'  => array(),
		        'title' => array(),
		...
		foreach ( $minimum_required_parameters as $key => $min_arr ) {
			...
			if ( $defined_value_int < $min_arr[0] && $defined_value_int !== -1 ) {
		...
		$edit_msg = __( 'How to fix this: You can edit your php.ini file to increase the specified variables to the recommanded values or you can ask your hosting company to make the changes for you.', 'neon-product-designer' );

		if ( ! empty( $message ) ) {
			echo '<div class="error">
	                   <p><b>Neon Product Designer: </b><br>' . wp_kses($message,$allowed_html) . '<br>
	                       <b>' . wp_kses($edit_msg, $allowed_html) . '</b></p>
	                </div>';
		}
